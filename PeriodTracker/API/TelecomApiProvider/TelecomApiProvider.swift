...
//  SecondPhone
//
//  Created by Konstantin Golenkov on 07.11.2023.
//
    ...
    private let provider: MoyaProvider<Target> = {
        var plugins: [PluginType] = [
            AccessTokenPlugin(tokenClosure: { _ in
                ...
                var accountId = accountProvider.accountId
                var accessToken = "AT-\(accountProvider.accessToken)"
                if UIDevice.current.isSandbox {
                    accountId = "stg_\(accountId)"
                    accessToken = "stg_\(accessToken)"
                }
#if DEBUG
